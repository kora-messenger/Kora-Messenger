...
class _AccountScreenState extends State<AccountScreen> {
  ...
  Future<void> _loadSession() async {
    ...
    if (session != null) {
      // Fetch fresh profile from backend to ensure isVerified and
      // other fields are up-to-date (not stale from an old session).
      try {
        ...
        if (userId != null && userId.isNotEmpty) {
          final response = await http.post(
            ...
            body: jsonEncode({'action': 'getProfile', 'userId': userId}),
          ).timeout(const Duration(seconds: 10));
          final data = jsonDecode(response.body);
          if (data['success'] == true && data['user'] != null) {
            final freshUser = data['user'] as Map<String, dynamic>;
            await SessionManager.instance.saveSession(freshUser);
            if (mounted) {
              setState(() {
                _session = freshUser;
                _loading = false;
        ...
        // Fall back to stored session if backend is unreachable
      }
    }
    if (mounted) {
      setState(() {
