...
class _ProfileSetupScreenState extends State<ProfileSetupScreen> {
  ...
  Future<void> _checkUsernameAvailability(String username) async {
    final UsernameCheckResult result = await _auth.checkUsername(username);
    if (!mounted) return;
    setState(() {
      ...
      try {
        ...
        final uploadResp = await http.post(
          ...
          body: jsonEncode({
            ...
            'fileName': 'avatar_$userId.${_photo!.path.endsWith('.webp') ? 'png' : 'jpg'}',
            'fileType': _photo!.path.endsWith('.webp') ? 'image/png' : 'image/jpeg',
          }),
