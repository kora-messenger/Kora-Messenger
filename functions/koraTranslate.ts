...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!text || !text.trim()) {
      return jsonResponse({ success: false, error: 'Missing or empty "text" field.', translatedText: null }, 400);
    }
    if (!targetLang) {
      return jsonResponse({ success: false, error: 'Missing "targetLang" field.', translatedText: null }, 400);
    }
    ...
    return jsonResponse({ success: false, error: String(e), translatedText: null }, 500);
  }
});
